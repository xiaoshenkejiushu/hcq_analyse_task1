...
def drop_columns(X, predict=False):#删除掉某些特定的列
    columns = [
...
def get_preprocess_data(predict=False):#对读进来的数据做基本的填充处理
    dir_path = 'E:/code2019/o2o_tianchi/data/'

    if predict:
        offline = pd.read_csv(dir_path+'ccf_offline_stage1_test_revised.csv', parse_dates=['Date_received'])
    else:
        offline = pd.read_csv(dir_path+'ccf_offline_stage1_train.csv', parse_dates=['Date_received', 'Date'])
    ...
    # online
    online = pd.read_csv(dir_path+'ccf_online_stage1_train.csv', parse_dates=['Date_received', 'Date'])
...
def get_offline_features(X, offline):#得到off-line特征
    # X = X[:1000]
...
def detect_duplicate_columns():#去除重复列
    X = get_train_data()
    X = X[:1000]
...
def grid_search_xgb(get_param=False):#网格搜索xgb
    params = {
        # all
        # 'learning_rate': 1e-1,
        # 'n_estimators': 80,
...
def train_xgb(model=False):#xgb训练
    global log
...
def feature_importance_score():#特征重要性评分
    clf = train_xgb()
